void parseUDP()
{
  ...
  if (packetSize)
  {
    ...
    #ifdef USE_MQTT                                          // отправка ответа выполнения команд по MQTT, если разрешено
    if (espMode == 1U)
    {
...
void processInputBuffer(char *inputBuffer, char *outputBuffer, bool generateOutput)
{
    ...
    else if (!strncmp_P(inputBuffer, PSTR("BRI"), 3))
    {
      memcpy(buff, &inputBuffer[3], strlen(inputBuffer));   // взять подстроку, состоящую последних символов строки inputBuffer, начиная с символа 4
      if(lampMode==MODE_DEMO) {
        GlobalBrightness = constrain(atoi(buff), 1, 255);
        EepromManager::SaveGlobalBrightness(&GlobalBrightness);
        FastLED.setBrightness(GlobalBrightness);   
      } else {
    ...
    else if (!strncmp_P(inputBuffer, PSTR("P_ON"), 4))
    {
      ...
      loadingFlag = true;
      settChanged = true;
      lampMode = MODE_NORMAL; // сбросим демо при включении из приложения
      eepromTimeout = millis();
      changePower();
      sendCurrent(inputBuffer);
    ...
    else if (!strncmp_P(inputBuffer, PSTR("EFF_ENUM"), 8))  // перечисление эффектов
    {
      //TODO: написать :)
    }
...
void sendCurrent(char *outputBuffer)
{
  sprintf_P(outputBuffer, PSTR("CURR %u %u %u %u %u %u"),
    currentMode,
    (lampMode==MODE_DEMO ? GlobalBrightness : modes[currentMode].Brightness),
