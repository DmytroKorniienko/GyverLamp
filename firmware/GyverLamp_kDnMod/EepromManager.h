 ...
 * 142-193   53       если добавить ещё 5 эффектов, начальный адрес неиспользуемой памяти сдвинется с 142 на 147
*/
...
#define EEPROM_GLOBAL_BRIGHTNESS             (194U)         // адрес в EEPROM памяти для хранения глобальной яркости
...
class EepromManager
{
  public:
    static void InitEepromSettings(ModeType modes[], AlarmType alarms[], uint8_t* espMode, bool* onFlag, uint8_t* dawnMode, int8_t* currentMode, bool* buttonEnabled,
      void (*readFavoritesSettings)(), void (*saveFavoritesSettings)(), byte* globalBrightness)
    {
      ...
      if (EEPROM.read(EEPROM_FIRST_RUN_ADDRESS) != EEPROM_FIRST_RUN_MARK)
      {
        ...
        EEPROM.write(EEPROM_GLOBAL_BRIGHTNESS, BRIGHTNESS);
    ...
    static void SaveGlobalBrightness(byte* globalBrightness)
    {
      EEPROM.write(EEPROM_GLOBAL_BRIGHTNESS, *globalBrightness);
      EEPROM.commit();
    }
