...
#define INVERSE_ALARM_TIME    (true)                        // нужно ли инвертировать вывод времени т.е. цветом по пустому фону (false) или пустым фоном по заливке (true)
boolean textinverse =         (false);                      // дефолтное состояние, либо false, либо INVERSE_ALARM_TIME
...
//#define NEWYEAR_MESSAGE                                     // вывод поздравления с новым годом и обратным отсчетом
                                                            // дата/время в UNIX формате, см. https://www.cy-pr.com/tools/time/ , 1609459200 => Fri, 01 Jan 2021 00:00:00 GMT

#ifdef NEWYEAR_MESSAGE
#define NEWYEAR_UNIXDATETIME 1609459200
...
char *msg2 = "C новым %d годом!";
char strMessage[256] = "До нового года осталось %d %s.";    // буфер на 256 символов
float calc;
boolean isStrPrepated = false;
#endif
...
static timerMinim tmDemoTimer(33*1000);                     // смена эффекта в демо режиме по дабл-клику из выключенного состояния, таймаут N секунд
